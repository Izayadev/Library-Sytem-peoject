...
class Main(QMainWindow , MainUI):
    ...
    def Handle_Login(self):
        # Get Login Form data
        self.cur.execute("select username, password, its_me, error from login where id = {}".format(
            login1.id
        ))
        beta = self.cur.fetchone()
        ...
        if username=='admin' and password=='000':
            ...
            # Check data to login
            self.cur.execute("select password, name, Branch from employee where name = '{}' and Password = '{}'".format(username, password))
            ...
            if len( data ) != 0:
                ...
                employee_id = data[0][1]
                branch_id = data[0][2]
                print("branch_id="+str(branch_id))

            
                # Get data from database
                self.cur.execute("select * from employee_permission where name = '{}'".format(username))
        ...
        if its_me==True: # just my own user UwU
            print("Here Too")
        ...
        elif error!=1: ###### Login as normal
            if data[24]==1: # Admin -> All permissions
                # Side Buttons
                self.pushButton_2.setEnabled(1)
                self.pushButton_6.setEnabled(1)
                self.pushButton_5.setEnabled(1)
            ...
            if data[12]==1: 
                ...
                if data[5]==1:# Check Export  
                    self.pushButton_11.setEnabled(1)
                if data[6]==1: # Check Import
                    self.pushButton_12.setEnabled(1)


                if data[2]==1 :# Check Add Book
                    self.tabWidget_2.setTabVisible(1,1)
                
                if data[3]==1 :# Check Edit Book
                    self.tabWidget_2.setTabVisible(2,1)
                    self.pushButton_15.setEnabled(1)
                    
                    if data[4]==1 : # Check delete book
                        self.pushButton_16.setEnabled(1)
            ...
            if data[13]==1: 
                ...
                if data[10]==1:# Check Export  
                    self.pushButton_45.setEnabled(1)
                if data[11]==1: # Check Import
                    self.pushButton_44.setEnabled(1)


                if data[7]==1 :# Check Add Client
                    self.tabWidget_3.setTabVisible(1,1)
                
                if data[8]==1 :# Check Edit Client
                    self.tabWidget_3.setTabVisible(2,1)
                    self.pushButton_29.setEnabled(1)
                    
                    if data[9]==1 : # Check delete Client
                        self.pushButton_30.setEnabled(1)
            ...
            if data[17]==1:
                ...
                if data[22]==1: # Add employee
                    self.tabWidget_4.setTabVisible(1,1)
                
                if data[23]==1: # Edit Employee
                    self.pushButton_36.setEnabled(1)
                    self.pushButton_41.setEnabled(1)

                if data[18]==1: # Add branch
                    self.pushButton_24.setEnabled(1)
                
                if data[19]==1: # Add publisher
                    self.pushButton_25.setEnabled(1)
                
                if data[20]==1: # Add author
                    self.pushButton_31.setEnabled(1)
                
                if data[21]==1: # Add category
                    self.pushButton_32.setEnabled(1)
            ...
            self.cur.execute("select * from history where id = '{}'".format(history_id))
            ...
            self.cur.executemany("insert into history values (?,?,?,?,?,?)",[(
                history_id, employee_id, "Login", date, branch_id, username
            )])
